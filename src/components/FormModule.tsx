...
import GSTTableCompanyDetails from "./common-master/GSTTableCompanyDetails";
import { ModularGSTTable } from "./common-master/ModularGSTTable";
...
function FormModule<T extends Record<string, any>>({
  ...
  const watchCustomerName = form.watch("CustomerName" as Path<T>);
  const watchCostCenterName = form.watch("CostCenterName" as Path<T>);
  const watchWareHouseName = form.watch("WareHouse" as Path<T>);
      ...
      >
        ...
        >
          {formFields.map((item) => {
            ...
            if (item.type === "date") {
              return (
                <DatePickerField key={item.name} item={item} editForm={form} />
              );
            }
            // if (item.type === "date") {
            //   return (
