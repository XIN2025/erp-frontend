...
interface GSTTableCompanyDetailsProps {
  data: GSTTableData[];
  setData: (newData: GSTTableData[]) => void;
}

const GSTTableCompanyDetails: React.FC<GSTTableCompanyDetailsProps> = ({ data, setData }) => {
  const addRow = () => {
    setData([
      ...data,
      {
  ...
  return (
    <div style={{ width: "90%" }} className="border mx-auto border-stone-700 rounded-lg p-1 overflow-hidden">
      <Table>
        <TableHeader>
          <TableRow>
            {formTableHeaders.map((header) => (
                ...
                <TableCell key={header.id}>
                  {header.id === "SerialNo" ? (
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        const updatedData = [...data];
                        updatedData[index][header.id] = e.target.value;
                        setData(updatedData);
                      }}
                    />
                  )}
...
export default GSTTableCompanyDetails;